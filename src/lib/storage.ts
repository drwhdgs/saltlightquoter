...
const ultraDecodeAndDecompress = (
  encoded: string
): { client: Client; packages: Package[]; createdAt: string } => {
  try {
...
// -------------------- Exports --------------------
export const decodeQuoteFromUrl = (
  encoded: string
): { client: Client; packages: Package[]; createdAt: string } | null => {
  if (encoded === 'error') return null;
  try {
    return ultraDecodeAndDecompress(encoded);
  } catch (err) {
...
export const generateEmailToClient = (quote: Quote): string => {
  ...
  const totalMonthly = quote.packages.reduce(
    (sum, pkg) => sum + pkg.totalMonthlyPremium,
    0
  );
...
${quote.packages
  .map(
    (pkg, i) => `Package ${i + 1}: ${pkg.name}
Monthly Premium: $${pkg.totalMonthlyPremium.toLocaleString()}
Plans Included: ${pkg.plans.map((p) => p.name).join(', ')}`
  )
  .join('\n\n')}
