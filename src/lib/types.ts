...
export interface CalendarEvent {
  ...
  title: string;
  date: string;
  type: "follow-up" | "renewal" | "birthday" | "marketing";
}
...
// Set to 12 PM to prevent timezone issues
...
export const getFirstOfNextMonth = (): string => {
  ...
  firstNextMonth.setHours(12, 0, 0, 0);
  return firstNextMonth.toISOString().split("T")[0];
};

// ✅ FIXED LOGIC: Only sets effectiveDate if type is 'health', otherwise, it is omitted.
export const withEffectiveDate = (
  ...
  if (type === "health") {
    // Only 'health' plans get the 1st of next month date.
    return { ...plan, effectiveDate: getFirstOfNextMonth() };
  }
  
  // All other plan types do not have an effectiveDate set on the plan object.
  return { ...plan };
};
...
export const PACKAGE_TEMPLATES: PackageTemplate[] = [
  {
    ...
    defaultPlans: [
      withEffectiveDate({
        ...
        coinsurance: 30,
        details:
          "Offers essential health coverage with the lowest monthly premiums but higher out-of-pocket costs, ideal for individuals who want protection from major medical expenses while keeping monthly costs affordable.",
        primaryCareCopay: 0,
        specialistCopay: 0,
        genericDrugCopay: 0,
      }),
  ...
  },
  {
    ...
    defaultPlans: [
      withEffectiveDate({
        ...
        deductible: 2500,
        details:
          "Sedera Health provides an innovative alternative to traditional health insurance through community cost sharing.",
        brochureUrl:
      ...
      withEffectiveDate({
        ...
        details:
          "Get your care needs where you want it at your home or work.",
        brochureUrl:
